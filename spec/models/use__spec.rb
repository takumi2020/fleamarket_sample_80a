...
describe User do
  describe '#create' do
    ...
    it "family_nameが半角ならNG" do
      user = build(:user, family_name: "hogehoge")
      user.valid?
      expect(user.errors[:family_name]).to include("は不正な値です")
    end
    it "last_nameが半角ならNG" do
      user = build(:user, last_name: "hoge")
      user.valid?
      expect(user.errors[:last_name]).to include("は不正な値です")
    end
    it "family_name_kanaが半角ならNG" do
      user = build(:user, family_name_kana: "hoge")
      user.valid?
      expect(user.errors[:family_name_kana]).to include("は不正な値です")
    end
    it "last_name_kanaが半角ならNG" do
      user = build(:user, last_name_kana: "hoge")
      user.valid?
